...
float4 PS_main(VS_OUT input) : SV_TARGET
{
	...
	//float4 lum = MaxLumSRV.Sample(SamplerWrap, float2(0.0,0.0));
	//float3 finalLum = RGBEToRGB(lum);

	//return float4(finalLum, 1.0);
	...
	return MaxLumSRV.Sample(SamplerWrap, input.Tex);
	//return AvgLumSRV.SampleLevel(SamplerWrap, input.Tex, levels.x);
